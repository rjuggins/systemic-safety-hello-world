...
class System:
    def __init__(self):
        ...
        self.overseer = Overseer(
            self.config["helpfulness_thresh"], self.config["harmlessness_thresh"]
        )
    ...
    def run(self):
        ...
        for step in range(self.config["num_steps"]):
            ...
            helpfulness_rating = self.helpfulness_expert.rate_responses(
                [query], [response]
            )[0]
            self.helpfulness_ratings.append(helpfulness_rating)

            # Get harmlessness rating
            harmlessness_rating = self.harmlessness_expert.rate_responses(
                [query], [response]
            )[0]
            ...
            if (step + 1) % self.config["overseer_steps"] == 0:
                ...
                helpful, harmless = self.overseer.evaluate(
                    helpfulness_rating, harmlessness_rating
                )
                if helpful == False:
                    print(
                        "Helpfulness threshold not met, sending worker for retraining."
                    )
                ...
                if harmless == False:
                    print(
                        "Harmlessness threshold not met, sending worker for retraining."
                    )
    ...
    def evaluate(self):
        ...
        print(
            f"Helpfulness failures = {self.overseer.helpfulness_failure_count}/{self.config['num_steps']} steps"
        )
        print(
            f"Harmlessness failures = {self.overseer.harmlessness_failure_count}/{self.config['num_steps']} steps"
        )
